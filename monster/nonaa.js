var nonaa = (function() {
    ...
    my.setPixel = function(ctx, x, y) {
        ctx.fillStyle = 'black';
        ctx.fillRect(x,y,1,1);
    };

    my.plotLine = function(ctx, x0, y0, x1, y1)
    {
    ...
    my.fillCircle = function(ctx, xm, ym, r)
    {
        xm = Math.floor(xm);
        ym = Math.floor(ym);
        r = Math.floor(r);
        ...
        do {
            //my.setPixel(ctx, xm-x, ym+y);                            /*   I. Quadrant +x +y */
            //my.setPixel(ctx, xm-y, ym-x);                            /*  II. Quadrant -x +y */
            //my.setPixel(ctx, xm+x, ym-y);                            /* III. Quadrant -x -y */
            //my.setPixel(ctx, xm+y, ym+x);                            /*  IV. Quadrant +x -y */

            my.plotLine(ctx, xm-x, ym+y, xm-x, ym);
            my.plotLine(ctx, xm-y, ym-x, xm-y, ym);
    ...
    my.plotCircleTheirs = function(ctx, xm, ym, r)
    {
        xm = Math.floor(xm);
        ym = Math.floor(ym);
        r = Math.floor(r);
    ...
    my.plotCircle = function(ctx, xm, ym, r)
    {
        xm = Math.floor(xm);
        ym = Math.floor(ym);
        r = Math.floor(r);
        ...
        var x = -r;
        var y = 0;
        var err = -r * 2 + 2; // 2-2*r;                /* bottom left to top right */
        ...
        if (xx*(x2-x1) > 0) {                             /* horizontal cut at P4? */
            if (yy*(y2-y1) > 0)                          /* vertical cut at P6 too? */
                if (Math.abs(xx*y) > Math.abs(yy*x)) {               /* which first? */
                    x0 = x2; x2 = xx+x1; y0 = y2; y2 = yy+y1;          /* swap povars */
                }                            /* now horizontal cut at P4 comes first */
    ...
    {                     /* rectangle enclosing the ellipse, eger rotation angle */
    ...
    {                                              /* plot any cubic Bezier curve */
        ...
        for (i = 1; i < n; i++)                         /* bubble sort of 4 povars */
            if ((t1 = t[i-1]) > t[i]) { t[i-1] = t[i]; t[i] = t1; i = 0; }

        t1 = -1.0; t[n] = 1.0;                                /* begin / end povar */
