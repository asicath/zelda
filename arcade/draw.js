...
var DemoDraw = (function() {
    ...
    var displayPlayerInfo = function(ctx, playerId, x, player) {
        ...
        while (i > 0) {
            ...
            Sprites.heart[index].drawSprite(ctx, x + (i/4)*8, y, Palettes.DeathStarRedBlue);
            i-=4;
        }

        y+=8;
        View.drawText(ctx, " x" + (player.rupees || 0) + "", x, y);
        Sprites.icons[0].drawSprite(ctx, x, y, Palettes.Icon);

        View.drawText(ctx, " x" + (player.bombs || 0) + "", x+8*6, y);
        Sprites.icons[2].drawSprite(ctx, x+8*6, y, Palettes.IconBlue);
